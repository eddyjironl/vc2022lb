...
@section("contenido")
    ...
    <form action="{{ route('guardar_cgmic4') }}" method="POST">
        <div class="contenedor mb-2 barra_info mt-4">
            <strong>Agrupacion de Catalogo 4 </strong> <br>
        ...
        <div class="container">
            <div class="form-group">
                ...
                <input type="text" name="cmic4no" id="cmic4no" class="form-control" value="{{ old('cmic4no') }}" placeholder="Indique un codigo">
    ...
    <div class="conteiner mt-5">
        ...
        <tbody>
            @foreach($odata as $cuenta)
                <tr>
                    <th scope="row"> {{ $cuenta->cmic4no }} </th>
                    <td> {{ $cuenta->cdesc }}</td>
                    <td> {{ $cuenta->mnotas }}</td>
                    <td>
                        <a href="{{ route('editar_cgmic4',$cuenta)}}" class="btn btn-warning btn-sm">Editar</a>
                        <form action="{{route('borrar_cgmic4',$cuenta) }}" class="d-inline" method="post">
                            @method("DELETE")
                            @csrf
