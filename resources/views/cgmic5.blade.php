...
@section("contenido")
    <form action="{{ route('guardar_cgmic5') }}" method="POST">
        ...
        <div class="container">
            <div class="form-group">
                ...
                <input type="text" name="cmic5no" id="cmic5no" class="form-control" value="{{ old('cmic5no') }}" placeholder="Indique un codigo">
    ...
    <div class="conteiner mt-5">
        ...
        <tbody>
            @foreach($odata as $cuenta)
                <tr>
                    <th scope="row"> {{ $cuenta->cmic5no }} </th>
                    <td> {{ $cuenta->cdesc }}</td>
                    <td> {{ $cuenta->mnotas }}</td>
                    <td>
                        <a href="{{ route('editar_cgmic5',$cuenta)}}" class="btn btn-warning btn-sm">Editar</a>
                        <form action="{{route('borrar_cgmic5',$cuenta) }}" class="d-inline" method="post">
                            @method("DELETE")
                            @csrf
